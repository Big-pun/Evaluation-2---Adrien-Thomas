<template>
  <div class="carte-livre p-4 bg-white shadow-md rounded-lg max-w-xs mx-auto text-center">
    ...
    <button @click="changerEtatLectureHandler" class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
      {{ livre.lu ? 'Marquer comme non lu' : 'Marquer comme lu' }}
    </button>
    <button @click="confirmerSuppression" class="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600 ml-2">
      Supprimer
    </button>
  </div>
</template>
